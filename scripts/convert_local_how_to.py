...
# Paths
wiki_source = Path("/sessions/compassionate-gallant-pasteur/mnt/Machine Learning Knowledge Garden/wiki/localhowto")
website_dest = Path("/sessions/compassionate-gallant-pasteur/mnt/Machine Learning Knowledge Garden/website/local-how-to")
website_dest.mkdir(parents=True, exist_ok=True)

# Silo definitions
silo = "Local How-To"
silo_name = "Local How-To"

# Files to publish immediately
published_files = {
    # All entries start as review
}

def assess_difficulty(content, title):
    """Estimate difficulty level based on content complexity"""
    beginner_keywords = ["introduction", "basic", "simple", "fundamentals", "what is", "overview", "getting started"]
    advanced_keywords = ["advanced", "complex", "theory", "mathematical", "optimization", "profiling", "tuning"]
...
def extract_soulcraft_theme(content):
    ...
    if "local" in content.lower() or "setup" in content.lower() or "practical" in content.lower():
        return "practical-emergence"
    return ""
...
# Main conversion
print("Converting Local How-To entries...")
entries = []
published_count = 0
review_count = 0
...
with open('local_how_to_entries.json', 'w') as f:
    json.dump(entries, f, indent=2)

print("\nEntry info saved to local_how_to_entries.json")
