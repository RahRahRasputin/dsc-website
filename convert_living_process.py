...
SRC = r"C:\Users\brad1\Documents\Claude\Projects\Machine Learning Knowledge Garden\website in markdown\garden\living-process"
DST = r"C:\Users\brad1\Documents\Claude\Projects\Machine Learning Knowledge Garden\website\pages\wiki\living-process"
HUB_COLOR = "#2980b9"

DIFFICULTY_MAP = {
    'attention-mechanisms': 'Beginner', 'beam-search-and-decoding-strategies': 'Beginner',
    'embeddings': 'Beginner', 'logit-processors-and-constrained-decoding': 'Beginner',
    'positional-encoding': 'Beginner', 'softmax-and-output-layer': 'Beginner',
    'temperature-and-sampling': 'Beginner', 'the-context-window': 'Beginner',
    'tokenization': 'Beginner',
    'continuous-batching-and-iteration-level-scheduling': 'Intermediate',
    'prefill-vs-decode-phases': 'Intermediate', 'speculative-decoding-inference': 'Intermediate',
}

LIVING_PROCESS_SLUGS = {
    'adaptive-batch-sizing-during-inference','adaptive-computation-and-early-exit-patterns',
    'attention-mechanisms','beam-search-and-decoding-strategies',
    'continuous-batching-and-iteration-level-scheduling','embeddings',
    'forward-pass-profiling-and-bottleneck-diagnosis','gpu-utilization-metrics-and-inference-monitoring',
    'inference-time-scaling','kv-cache','logit-processors-and-constrained-decoding',
    'positional-encoding','prefill-vs-decode-phases','process-reward-models-and-outcome-reward-models',
    'request-prioritization-and-scheduling-policies','softmax-and-output-layer',
    'speculative-decoding-inference','starvation-prevention-in-scheduling',
    'temperature-and-sampling','the-context-window','token-healing-and-tokenization-artifacts',
    'tokenization','training-vs-inference'
}
...
def inline_md(text):
    ...
    text = re.sub(r'\[\[([^\]]+)\]\]', lambda m: slug_to_title(m.group(1).split('/')[-1]), text)
...
def extract_link_section(text):
    ...
    for line in links_text.split('\n'):
        ...
        if m:
            ...
            display = m.group(2) or slug_to_title(slug)
...
def make_see_also(links):
    ...
    for slug, display, desc in links:
        if slug and slug in LIVING_PROCESS_SLUGS:
            link_html = f'<a href="/wiki/living-process/{slug}/">{display}</a>'
        elif slug:
...
<head>
  ...
  <style>
    ...
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    .see-also {{ background: #f0f7fc; padding: 20px; border-left: 4px solid var(--hub-color); border-radius: 4px; margin-top: 30px; }}
...
<body>
  ...
  <div class="container">
    ...
    <div class="breadcrumb">
      <a href="/wiki/">Wiki</a> /
      <a href="/wiki/living-process/">Living Process</a> /
      {title}
    </div>
